...
import SideBarManuData from "../data/SideBarManuData.json";
import classNames from "classnames";

const SideBar = () => {
  const [sideBarManuData, setSideBarManuData] = useState(SideBarManuData);

  const ManuHeading = ({ eachManu, index }) => {
    return (
      <li key={eachManu.title} className="nav-header">
        {`${eachManu.title} ${index}`}
      </li>
    );
  };

  const ExpandableManuItem = ({ eachManu }) => {
    const [isExpanded, setIsExpanded] = useState(false);

    const handleExpand = () => {
      if (!eachManu.isNested) {
        return;
      }
    ...
    return (
      <li
        className={classNames("nav-item", {
          "menu-is-opening menu-open": isExpanded,
        })}
      >
        <a href={eachManu.link} className="nav-link" onClick={handleExpand}>
          <i className={`nav-icon far ${eachManu.icon}`}></i>
          <p>
            {eachManu.title}
            {eachManu.isNested && <i className="right fas fa-angle-left"></i>}
          </p>
        </a>
        ...
        {eachManu.isNested && (
          ...
          >
            {renderManuData(eachManu.nestedItem)}
          </ul>
        )}
      </li>
    );
  ...
  const renderManuData = (manuData) => {
    return manuData.map((eachManu, index) => {
      return eachManu.type == "heading" ? (
        <ManuHeading key={eachManu.title} eachManu={eachManu} index={index} />
      ) : (
        <ExpandableManuItem key={eachManu.title} eachManu={eachManu} />
      );
    });
  };
      ...
      <div className="sidebar">
        ...
        <nav className="mt-2">
          <ul
            ...
            data-accordion="false"
          >
            {renderManuData(sideBarManuData)}
          </ul>
        </nav>
        {/* /.sidebar-menu */}
      </div>
